...
class Charts {
  ...
  async salesData(options = { range: 'week' }) {
    try {
      ...
      return toPairs(groupBy(weeklyTickets.items, (ticket) => new Date(ticket.created_at).toLocaleDateString())).map(([day, tickets]) => ({ day, amount: tickets.reduce((amount, ticket) => amount + Number(ticket.totalAmount), 0) }));
  ...
  async ticketsData(options = { range: 'week' }) {
    try {
      const weeklyTickets = await this.ticket.query(this.ticket.queries.weeklyTickets());
      const groupByTickets = toPairs(groupBy(weeklyTickets.items, (ticket) => new Date(ticket.created_at).toLocaleDateString()));
      ...
      return groupByTickets.map(([day, tickets]) => {
        ...
        const { median, position, sorted: ticketsTotalAmountsSorted } = this.median(ticketsTotalAmounts);
  ...
  async stockData() {
    try {
      ...
      // @todo group by brand ?
      return stockItems.map((stockItem) => ({ amount: stockItem.amount, price: stockItem.price, sold: stockItem.sold, title: this.stock.formatDescription(stockItem) }));
    } catch (e) {
      // throw new Error(e, options);
      return [];
    }
  ...
  median(list) {
    ...
    if (!sortedList[medianIndex]) return { median: sortedList[0], position: medianIndex, sorted: sortedList };
